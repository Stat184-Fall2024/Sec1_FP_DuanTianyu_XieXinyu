...
summary_stats <- clothing_data %>%
  summarise(
    ...
    avg_rating = mean(Review.Rating, na.rm = TRUE),
    sd_rating = sd(Review.Rating, na.rm = TRUE)
  )
...
model1_1 <- lm(
  Purchase.Amount..USD. ~ Review.Rating + Season + Review.Rating + Frequency.of.Purchases + Promo.Code.Used, 
 data = clothing_data
)
...
model_results <- tidy(model1_1)  # Use tidy() from the broom package to extract model results

# Select required columns: variable names, Beta coefficients, and P-values
...
kable(beta_pvalue_table, 
      ...
      booktabs = TRUE)       # Beautify table for LaTeX
```
...
# 1. Separate numeric and factor variables
...
# 2. Calculate correlation matrix for numeric variables
library(corrplot)
cor_matrix <- cor(
  numeric_clothing, 
  method = "pearson"
...
# 3. Scatterplot matrix for numeric variables
pairs(
  numeric_clothing, 
  main = "Scatterplot Matrix of Numerical Variables"
)

# 4. Boxplot for the relationship between Card and Amount
library(ggplot2)
boxplot_plot <- ggplot(
  data = Clothing,
  mapping = aes(
...
# 5. Grouped summary statistics for Card
library(dplyr)
...
# 6. ANOVA to test the significance of Card on Amount
anova_result <- aov(
  Amount ~ Card, 
  data = Clothing
  )
...
best_subset_Cloth_lm <- lm(
  formula = Amount ~ Card + Dollar24 + Freq24, 
  data = Clothing
  )
...
#(k)
model_Cloth_k <- lm(
  formula = Amount ~ AvgSpent12 + Card, 
  data = Clothing
  )
...
#(l)
model_M1 <- lm(
  formula = log(Amount) ~ log(AvgSpent12) + Card, 
  data = Clothing
  )
summary(model_M1)
